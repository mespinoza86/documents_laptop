...
if [ "$xxOsRevMajor" -ge 10 ]; then
    ...
    # Idea: Scan /home/marco/Documents/tesis/others/petalinux/dma-mapped/build/tmp/sysroots/plnx_arm/usr/include/sys/unistd.h for matches with
    ...
    # /home/marco/Documents/tesis/others/petalinux/dma-mapped/build/tmp/sysroots/plnx_arm/usr/include/sys/unistd.h -- and the CPU_* #defines will be kept
    # up to date with new CPU/OS releases.
    xxcpu=`getconf CPU_VERSION`; # Get the number.
    xxcpu=`printf '0x%x' $xxcpu`; # convert to hex
    archname=`sed -n -e "s/^#[[:space:]]*define[[:space:]]*CPU_//p" /home/marco/Documents/tesis/others/petalinux/dma-mapped/build/tmp/sysroots/plnx_arm/usr/include/sys/unistd.h |
...
EOM
	...
	if [ "$xxOsRevMajor" -eq 10 ]; then
	    ...
	    if [ -f /home/marco/Documents/tesis/others/petalinux/dma-mapped/build/tmp/sysroots/plnx_arm/usr/include/pthread.h ]; then
...
case "`grep 'double strtold.const' /home/marco/Documents/tesis/others/petalinux/dma-mapped/build/tmp/sysroots/plnx_arm/usr/include/stdlib.h`" in
...
case "`grep 'isfinite' /home/marco/Documents/tesis/others/petalinux/dma-mapped/build/tmp/sysroots/plnx_arm/usr/include/math.h`" in
*"isfinite"*) ;;
*) d_isfinite=undef ;;
esac
