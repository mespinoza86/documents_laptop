...
proc is_ip_interrupt_connected {srcname} {
	set intr_pins [hsi get_pins -filter "DIRECTION==O && TYPE==INTERRUPT" -of_objects [hsi get_cells ${srcname}]]
...
proc plnx_gen_conf_processor {mapping kconfprefix} {
	...
	foreach m ${mapping} {
		...
		set hds [hsi get_cells -filter IP_NAME==${ipname}]
		foreach hd ${hds} {
	...
	foreach cpu [concat ${aarch64list} ${armlist} ${mblist}] {
		...
		set kname [plnx_fix_kconf_name ${cpuname}]
	...
	if { "${cpuchoicesstr}" == "" } {
		puts stderr [format "%s\n%s\n%s\n" "No CPU can be found in the system."\
					...
					"Valid processors are: microblaze, ps7_cortexa9, psu_cortexa53."]
		error ""
	}
	set kconfstr [format "%s\n%s\n\t%s\n\t%s\n\t%s\n%s\n%s\n" "${kconfstr}" \
		"choice" \
	...
	foreach m ${mapping} {
		...
		foreach hd ${hds} {
			...
			if {"${has_bank}" == "n"} {
				...
				if {[regexp "ps[7u]_ddr" "${ipname}" match]} {
	...
	foreach m ${mapping} {
		...
		foreach hd ${hds} {
			...
			if {"${baudrateproperty}" != "" && "${is_baudrate_editable}" == "n"} {
				set baudrates [list [hsi get_property ${baudrateproperty} ${hd}]]
			} elseif {"${hcbaudrate}" != "" } {
			...
			set serialipname [format "%s\n%s\n%s\n%s\n%s\n" "${serialipname}"\
				"config ${serialkconfprefix}IP_NAME"\
				"string"\
				"default [string tolower ${name}]"\
				"depends on ${serialkconfprefix}${kname}_SELECT"]
			if { "${baseaddr_property}" != "" } {
...
proc plnx_get_conf_flash_partition {prefix flashname bankid advdepends} {
	...
	for {set i 0} {${i} < 20} {incr i} {
		...
		set flashipname [format "%s\n%s\n%s\n%s\n%s\n" "${flashipname}"\
			...
			"string"\
			"default [string tolower ${flashname}]"\
			"depends on ${namedepends}"]
	}
	...
	foreach m ${mapping} {
		...
		foreach hd ${hds} {
			...
			if {"${ipname}" == "axi_emc"} {
				...
				if {"${name}" == "ps7_sram_0"} {
					set nor_cs [hsi get_property CONFIG.C_NOR_CHIP_SEL0 [hsi get_cell "ps7_smcc_0"]]
				} else {
					set nor_cs [hsi get_property CONFIG.C_NOR_CHIP_SEL1 [hsi get_cell "ps7_smcc_0"]]
				}
				if {"${nor_cs}" == "0"} {
					continue
				}
...
proc generate_mapping_list {args} {
	...
	set devicetypes {processor memory serial ethernet flash sd rtc timer reset_gpio}
	set mappinglist {}
	foreach devtype ${devicetypes} {
		set devtype_mapping {}
		lappend devtype_mapping "${devtype}"
		if {"${devtype}" == "sd"} {
			lappend devtype_mapping "processor_ip ps7_cortexa9 psu_cortexa53"
...
proc plnx_gen_hwsysconf {args} {
	...
	if { [catch {hsi open_hw_design "${hdf}"} res] } {
	...
	foreach c [lreplace ${retcpus} 0 0] {
		...
		foreach s [split ${cpuslaves}] {
			if { [lsearch -index 0 ${retslaves} "${s}"] < 0 } {
				set sipname [hsi get_property IP_NAME [hsi get_cell "${s}"]]
...
foreach lib_file [glob -directory "${xsct_root_dir}/scripts/hsm/" xillib*.tcl ] {
	source $lib_file
}
