...
def prop_set(fp, data):
	...
	for key in data.keys():
		if key in ['SRC_URI_append','FILESEXTRAPATHS_append','inherit','export','KERNEL_IMAGETYPE_zynq']:
...
def deploy_append_set(fp, data):
	dstfolder = data['dst'].keys()[0]
	srcfolder = data['src'].keys()[0]
...
def create_layer_infile(prio):
	data = """
	{
	 \"create_example_recipe\": \"n\",
	 \"create_example_bbappend\": \"n\",
	 \"layer_priority\": \"%d\"
	}"""%(prio)
...
def create_append(lp, ln ,t):
	...
	obj.wait()
	if obj.returncode != 0:
		print "WARNING: No recipe "+t+" found "
		return None
...
def check_avail_of_all_targ(lp, ln, targets, pathfile):
	...
	if not paths:
		...
		for t in targets:
			if t in paths.keys():
				print "INFO: "+t+" append file exists"
			else:
				p = create_append(lp, ln, t)
				if p:
					paths[t] = p
...
def check_for_layer(lp, ln, prio, ta, pathfile, bdir):
	...
	if not os.path.exists(lp):
		print "ERROR: Path Dosent exists: "+lp
		exit(1)
		...
		prevdir=os.getcwd()
		os.chdir(lp)
		create_layer_infile(prio)
		cmd="yocto-layer create "+ln+" -i new.json"
		print "	%s"%(cmd)
		if subprocess.call(cmd.split(' ')):
			print "ERROR: Could not create layer "+ln
			exit(1)
		else:
			os.remove("new.json")
			os.chdir(prevdir)
...
def main(args):
	...
	for ta in targets:
		if not ta in file_paths.keys():
			print " WARNING:Skipping "+ta+" append file creation"
		else:
